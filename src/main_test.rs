...
fn main() -> iced::Result {
    ...
    if let Err(err) = diagnostics::emit(
        &mut stderr,
        &world,
        result.warnings.iter(),
        DiagnosticFormat::Human,
    ) {
        eprintln!("erro ao imprimir avisos: {err}");
    }
    ...
    let document = match result.output {
        ...
        Err(errors) => {
            ...
            {
                eprintln!("erro ao imprimir diagnósticos: {err}");
            }
            return Ok(());
        }
    };
    ...
    let pdf = match typst_pdf::pdf(&document, &PdfOptions::default()) {
        Ok(pdf) => pdf,
        Err(errors) => {
            eprintln!("falha ao exportar PDF: {} erro(s)", errors.len());
            return Ok(());
        }
    };
    ...
    if let Err(err) = std::fs::create_dir_all(OUT_DIR) {
        eprintln!("erro ao criar diretório {OUT_DIR}: {err}");
        return Ok(());
    }

    if let Err(err) = std::fs::write(&output, pdf) {
        eprintln!("erro ao escrever {}: {err}", output.display());
        return Ok(());
    }
